...
def goToProfile(request):
    currentuser = request.user.username if request.user.is_authenticated else ""
    profileuser = request.GET.get("user")
    print(f"currentuser: {currentuser} \nuserprofile: {profileuser}")
    author = User.objects.get(username=profileuser)
    return render(request, "network/profile.html", {
        "profile_user": author,
        "posts": Post.objects.filter(author=author).order_by('-timestamp'),
...
def likedpostcheck(request):
    if request.GET.get("id") and request.GET.get("user"):
        try: 
